...
using ParsAlphabet.ERP.Application.Dtos;
using ParsAlphabet.ERP.Application.Dtos.FM.Cahier;
using ParsAlphabet.ERP.Infrastructure.Implantation;

namespace ParsAlphabet.ERP.Sql.Queries.Chasier;

public class CashierRepository(
    IConfiguration config,
    IHttpContextAccessor accessor)
    : BaseRepository<CashierModel, int, string>(config)
{
    private readonly IHttpContextAccessor _accessor = accessor;

    public new static FormPlate1.GetColumnsViewModel GetColumns()
    {
        var list = new FormPlate1.GetColumnsViewModel
        {
            DataColumns = new List<FormPlate1.DataColumnsViewModel>
            {
                new()
                {
            ...
            Buttons = new List<FormPlate1.GetActionColumnViewModel>
            {
