    ...
    IBaseRepository<AdmissionCashPayment, long, string>
{
    ...
    public async Task<CSVViewModel<IEnumerable>> ExportCashSummeryUserCsv(GetAdmissionCashGrouByFundType model)
    {
        ...
        {
            Columns =
                "جریان کار,مرحله,نقدی,دستگاه پوز,فیش,حساب باز,حساب باز - کسر و اضافه صندوق پرسنل,حساب باز - خدمات اعتباری بیمار,حساب باز  -حق الزحمهطبیب,حساب باز - مساعده پرسنل,حساب باز - مددکاری,حساب باز - مابه تفاوت بیمه,حساب باز - خدمات اعتباری طبیب "
        };
