...
namespace ParsAlphabet.ERP.Infrastructure.Implantation.MC.AdmissionMasterCartable;

public class AdmissionMasterCartableRepository :
    BaseRepository<AdmissionMasterCartableGetPage, int, string>,
    IBaseRepository<AdmissionMasterCartableGetPage, int, string>
{
    ...
    public GetColumnsViewModel GetColumnsCartable()
    {
        var list = new GetColumnsViewModel
        {
            ...
            DataColumns = new List<DataColumnsViewModel>
            {
                ...
                new()
                {
                    Id = "admissionMasterAmount", Title = "مبلغ پرونده", Type = (int)SqlDbType.Decimal,
                    IsCommaSep = true, IsDtParameter = true, Width = 11
                },
                new()
                {
                    Id = "cashAmount", Title = "مبلغ صندوق", Type = (int)SqlDbType.Decimal, IsCommaSep = true,
                    IsDtParameter = true, Width = 11
                },
        ...
        list.Buttons = new List<GetActionColumnViewModel>
        {
            new()
            {
                Name = "previousMasterAction", Title = "گام قبلی", ClassName = "btn blue_outline_1",
                IconName = "fas fa-arrow-right"
            },
            new()
            {
                Name = "nextMasterAction", Title = "گام بعدی", ClassName = "btn blue_outline_1",
                IconName = "fas fa-arrow-left"
            },
            new()
            {
                Name = "inboundMasterservicelist", Title = "نمایش پرونده", ClassName = "btn blue_outline_1",
                IconName = "fa fa-list"
            },
            new()
            {
                Name = "admissionMasterBillPatientPrint", Title = "چاپ", ClassName = "btn blue_outline_1",
    ...
    {
        var fromWorkDayDate = model.Form_KeyValue[5]?.ToString().ToMiladiDateTime();
        var toWorkDayDate = model.Form_KeyValue[6]?.ToString().ToMiladiDateTime();

        var parameters = new DynamicParameters();
        parameters.Add("Id", model.Form_KeyValue[0]?.ToString() != null ? model.Form_KeyValue[0]?.ToString() : null);
        parameters.Add("StageId", model.Form_KeyValue[1]?.ToString() != null ? model.Form_KeyValue[1]?.ToString() : null);
        parameters.Add("WorkflowId", model.Form_KeyValue[2] != null ? model.Form_KeyValue[2] : null);
        ...
        parameters.Add("IsSettled",
            model.Form_KeyValue[7]?.ToString() != null ? model.Form_KeyValue[7]?.ToString() : null);
        using (var conn = Connection)
        {
            var sQuery = "[mc].[Spc_AdmissionMaster_Cartable_Section]";
            conn.Open();
            var result =
    ...
    public async Task<MyResultPage<List<AdmissionMasterCartableGetPage>>> GetCartable(NewGetPageViewModel model,
        byte roleId)
    {
        var result = new MyResultPage<List<AdmissionMasterCartableGetPage>>
        {
            Data = new List<AdmissionMasterCartableGetPage>()
        };
        ...
        using (var conn = Connection)
        {
            var sQuery = "[mc].[Spc_AdmissionMaster_Cartable]";
            conn.Open();
            result.Data =
                (await conn.QueryAsync<AdmissionMasterCartableGetPage>(sQuery, parameters,
    ...
    public async Task<int> GetWorkListCount(AdmissionMasterSectionCartable model, byte roleId)
    {
        var parameters = new DynamicParameters();
        ...
        using (var conn = Connection)
        {
            var sQuery = "[mc].[Spc_AdmissionMasterAction_WorkList_Count]";
            conn.Open();
