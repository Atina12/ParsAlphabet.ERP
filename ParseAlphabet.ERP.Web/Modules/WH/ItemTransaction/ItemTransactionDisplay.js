...
var viewData_getpagetableDisplay_url = `${viewData_baseUrl_WH}/ItemTransactionLineApi/display`,
    viewData_getlinepagetable_url = `${viewData_baseUrl_WH}/ItemTransactionLineApi/getitemTransactionlinepage`,
    viewData_getItemTransactionCheckExist = `${viewData_baseUrl_WH}/ItemTransactionApi/checkexist`,
    lastPageHeaderDisplayloaded = 0, activePageIdDisplay = "itemTransactionDisplayPage",
    display_PageRowsCount = 15;

function initDisplayItemTransactionForm(headerPagination = 0, id = 0) {
    ...
    pagetable_formkeyvalue[6] = $(`#${activePageIdDisplay} #isQuntity`).val();
    let pagetable = {};
    ...
    if (index1 == -1)
        ...
        arr_pagetables[index1] = pagetable
    ...
    if (index2 == -1)
        ...
        arr_pagetables[index2] = pagetable
    ...
    get_DisplayPageTable("headerDisplay_pagetable", id => {
        ...
        get_DisplayNewPageTable("lineDisplay_pagetable",
            function () {
                ...
                $("#titledisplayItemTransactionLineModel").text("")
                if (+pagetable_formkeyvalue[6] === 0)
                    $("#titledisplayItemTransactionLineModel").text("نمایش تعدادی");
                else
                    $("#titledisplayItemTransactionLineModel").text("نمایش ریالی");
                modal_show("displayItemTransactionLineModel");
...
function display_pagination_display(opr) {
    var elemId = +$("#itemTransactionDisplayPage #headerDisplay_pagetable #col_1_1").text();
...
async function display_paginationAsync_display(opr, elemId) {
    ...
    initDisplayItemTransactionForm(headerPagination, elemId)
}
...
function headerindexChoose_display(e) {
    ...
    if (e.keyCode === KeyCode.Enter) {
        ...
        checkExist = checkExistItemTransactionLineId(+elm.val());
        if (checkExist) {
            initDisplayItemTransactionForm(0, +elm.val());
        }
        else
...
function checkExistItemTransactionLineId(id) {
    ...
    let outPut = $.ajax({
        url: viewData_getItemTransactionCheckExist,
        async: false,
        type: "post",
        dataType: "json",
...
function fillItemTransactionDiaplaylineFooter(rowIdSelect) {

    getItemTransactionDiaplaylineFooter(rowIdSelect).then(function (result) {
        ...
        if (result.data != null) {
            $("#fillItemTransactionlineFooterTbodyfieldset").removeClass("displaynone");
            strTbodyTh = `
            <div class="detail-headerLine row col-md-12">
                <table style="margin-top:10px" id="fillItemTransactionlineFooterTbody">
                    <thead >
                        <tr>`
            ...
            $(strTbodyTh).appendTo("#fillItemTransactionlineFooterTbody");
        }

        else
            $("#fillItemTransactionlineFooterTbodyfieldset").addClass("displaynone");

    });
}

async function getItemTransactionDiaplaylineFooter(id) {
...
initDisplayItemTransactionForm();
