var viewData_form_title = "کارتابل اسناد انبار",
    viewData_controllername = "ItemTransactionCartableApi",
    viewData_getpagetable_url = `${viewData_baseUrl_WH}/${viewData_controllername}/getpageitemtransactioncartable`,
    viewData_getrecord_url = `${viewData_baseUrl_WH}/ItemTransactionApi/getrecordbyid`,
...
var stageId_ItemTransactionCartable = +$("#stageIdItemTransactionParameter").val();

pagetable_formkeyvalue = ["", "", "my", null];



function initItemTransactionGroup() {
    ...
    if ($("#userType").prop("checked")) {
        pagetable_formkeyvalue = [stageId_ItemTransactionCartable, "stageId", "my", 0];
    }
    else {
        pagetable_formkeyvalue = [stageId_ItemTransactionCartable, "stageId", "all", 0];
    }

    $('#userType').bootstrapToggle();

    getItemTransactionTab();
...
$("#userType").on("change", function () {
    ...
    if (stageId_ItemTransactionCartable > 0) {
        pagetable_formkeyvalue[0] = stageId_ItemTransactionCartable;
...
function callBackSearche() {
    return stageId_ItemTransactionCartable > 0
}

function getItemTransactionTab() {

    var byUser = pagetable_formkeyvalue[2];

    let url = `${viewData_baseUrl_WH}/${viewData_controllername}/itemtransactioncartablesection/3,4,8,11,14,15,16/${byUser}`;
...
function fillTab(result) {
    ...
    if (resultLen > 0) {
        ...
        let index = result.findIndex((item) => item.id === stageId_ItemTransactionCartable)
        if (index == -1) {
...
function fillLinkTab(result) {
    ...
    for (var i = 0; i < reasultLen; i++) {
        if (checkResponse(stageId_ItemTransactionCartable) && stageId_ItemTransactionCartable > 0)
            active = (result[i].id == stageId_ItemTransactionCartable ? "active" : "");
        else
            active = (i == 0 ? "active" : "");
...
function changeTabByClick(namePage, id) {
    ...
    stageId_ItemTransactionCartable = saveIdForTabs = +id;
    ...
    pagetable_formkeyvalue = [stageId_ItemTransactionCartable, "stageId", switchUser, 0];
...
function run_button_displaySimple(id, rowNo, elm) {
    ...
    var check = controller_check_authorize("ItemTransactionApi", "VIW");
    if (!check)
        return;
...
function run_button_printQuantity(transactionId, rowNo, elm, e) {
    ...
    var check = controller_check_authorize("ItemTransactionApi", "PRN");
    if (!check)
        return;

    let trElement = $(elm).parents("tr").first()
...
function run_button_editItemTransaction(id, rowNo, elem) {
    ...
    var check = controller_check_authorize("ItemTransactionApi", "UPD");
    if (!check)
        return;
    ...
    $(".modal").find("#modal_title").text("ویرایش اسناد انبار");
...
function run_button_itemTransactionDetailSimple(lineId, rowNo, elm, ev) {
    ...
    var check = controller_check_authorize("ItemTransactionApi", "INS");
    if (!check)
        return;
...
function run_button_showStepLogsItemTransaction(id, rowno, elm, ev) {
    
    ev.stopPropagation();
    activePageTableId = `pagetable_p_${stageId_ItemTransactionCartable}`;
...
function cartableModalClose(modalName) {
    ...
    stageId_ItemTransactionCartable = saveIdForTabs;

    if (modalName === "displayItemTransactionLineModel")
        activePageTableId = `pagetable_p_${stageId_ItemTransactionCartable}`;
