var viewData_allocate_Order = `${viewData_baseUrl_SM}/SalesPersonInvoiceLineApi/getpersonorderlist`,
    viewData_allocate_OrderLine = `${viewData_baseUrl_SM}/SalesPersonInvoiceLineApi/getpersonorderline`,
    viewData_allocate_Ins = `${viewData_baseUrl_SM}/SalesPersonInvoiceLineApi/allocatepersonorderline`,
...
function selectHeaderRow(id, orderTypeId, branchId, check, ev) {
    ...
    for (var i = 0; i < headerLen; i++) {
        var trHeader = $("#tempPersonOrder >tr")[i];
        if ($(trHeader).find("td:eq(0) > input:checkBox").prop("checked")) {
            checked = true;
            count += 1;
        }
        else
...
function selectHeaderLineRow(id, orderTypeId, branchId, itemTypeId, itemId, check, event) {
    ...
    var checkedline = false;
    var countLine = 0;
    for (var i = 0; i < lineLen; i++) {
        var trLine = $("#tempPersonOrderLine >tr")[i];
        if ($(trLine).find("td:eq(0) > input:checkBox").prop("checked")) {
            checkedline = true;
            countLine += 1;
        }
        else
    ...
    if (checkedline && countLine == lineLen) {
        countLine = 0;
        $("#checkAllLine").prop("checked", true);
    }
    if ($(check).prop("checked")) {
