...
var viewData_getpagetableDisplay_url = `${viewData_baseUrl_PU}/PurchaseOrderLineApi/display`,
    viewData_getlinepagetable_url = `${viewData_baseUrl_PU}/PurchaseOrderLineApi/getorderlinepage`,
    viewData_getPersonOrderCheckExist = `${viewData_baseUrl_PU}/PurchaseOrderApi/checkexist`,
    activePageIdDisplay = "purchasePersonOrderLineDisplayPage",
    lastPageHeaderDisplayloaded = 0;


function display_pagination_display(opr) {
  
    var elemId = +$("#purchasePersonOrderLineDisplayPage #headerDisplay_pagetable #col_1_1").text();
...
async function display_paginationAsync_display(opr, elemId) {
    ...
    initDisplayPersonOrderFrom(headerPagination, elemId)
}
...
function headerindexChoose_display(e) {
    ...
    if (e.keyCode === KeyCode.Enter) {
        ...
        if (checkExist) {
            initDisplayPersonOrderFrom(0, +elm.val());
        }
        else
...
function checkExistPersonOrderLineId(id) {
    ...
    let outPut = $.ajax({
        url: viewData_getPersonOrderCheckExist,
        async: false,
        type: "post",
        dataType: "json",
        ...
        error: function (xhr) {
            error_handler(xhr, viewData_getPersonOrderCheckExist);
        }
    });
...
function initDisplayPersonOrderFrom(headerPagination = 0, id = 0) {

    pagetable_formkeyvalue[0] = id == 0 ? $(`#${activePageIdDisplay} #personOrderId`).val() : id;
    ...
    if (arr_pagetables.findIndex(a => a.pagetable_id == "lineDisplay_pagetable") < 0) {
        pagetable = {
            ...
            getsum_url: `${viewData_baseUrl_PU}/PurchaseOrderLineApi/getLineSum`
        };
        arr_pagetables.push(pagetable);
        ...
        arr_pagetables[index2].getsum_url = `${viewData_baseUrl_PU}/PurchaseOrderLineApi/getLineSum`
    }
    ...
    get_DisplayPageTable("headerDisplay_pagetable", id => {
        ...
        get_DisplayNewPageTable("lineDisplay_pagetable",
            function () {
                modal_show("displayPersonOrderLineModel");
                //reset_toParentForm();
                ...
                for (var j = 0; j < columnsL; j++) {
                    ...
                    if (columns[j].isDtParameter && columns[j].id != "action") {
                        ...
                        if (columns[j].id == "action") {
                            ...
                            if (result.columns.actionType === "inline") {
                                ...
                                for (var k = 0; k < buttonsL; k++) {
                                    ...
                                    if (!btn.isSeparator) {
                                        ...
                                        if (btn.condition == null && btn.name == "averagePrices") {
                                            btn_tbidx++;
...
function run_button_averagePrices(lineId, rowNo, elem) {

    let pageId = "lineAveragePricesModal"
    $("#tempAveragePricesRows").html("");
    let url = `${viewData_baseUrl_PU}/PurchaseOrderLineApi/getrecordbyids`;
    $.ajax({
        url: url,
        type: "POST",
        dataType: "json",
        ...
        success: function (result) {
            if (result.data != null) {
                output = `<tr class="highlight" id="row${rowNo}" onkeydown="tr_onkeydownNewDisplay(${pageId},this,event)" >
...
function tr_onkeydownNewDisplay(pg_name, elem, ev) {
    ...
    if (ev.which === KeyCode.ArrowUp) {
        ...
        //بستن مدال نرخ میانگین
        else
            modal_close("averagePricesModal");
    }
