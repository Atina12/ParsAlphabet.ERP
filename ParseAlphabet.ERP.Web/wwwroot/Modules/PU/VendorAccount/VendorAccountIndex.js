var viewData_form_title = "حساب‌های تامین کننده";
var viewData_controllername = "VendorAccountApi";
...
var vendorAccountTypeId = 2;
var viewData_print_file_url = `${stimulsBaseUrl.PU.Prn}VendorAccount.mrt`;
var viewData_print_model = { url: viewData_print_file_url, item: "@Id", value: 0, sqlDbType: 8, size: 0 }
var viewData_print_tableName = "";


function initVendorAccountIndex() {
    inputMask();

    loadDropDownVendorAccount()

    pagetable_formkeyvalue = [0, vendorAccountTypeId];
    get_NewPageTableV1();
}

function loadDropDownVendorAccount() {
    fill_select2(`${viewData_baseUrl_PU}/VendorApi/getdropdown`, "personId", true, 0, false);
    fill_select2(`${viewData_baseUrl_FM}/BankApi/getdropdownisactive`, "bankId", true, 0, false);
}

function getPageTableFinished() {
    $("#filter_person").data("api", `${viewData_baseUrl_PU}/VendorApi/getdropdown`);
}
...
function saveVendorAccount() {
    ...
    saveVendorAccountAjax(url, model).then(function (result) {
...
async function saveVendorAccountAjax(url, model) {
...
$("#exportCSV").click(function () {
    ...
    csvModel = {
        ...
        Form_KeyValue: [0, vendorAccountTypeId]
    }
