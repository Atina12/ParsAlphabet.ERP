...
var viewData_getpagetableDisplay_url = `${viewData_baseUrl_PU}/PurchaseInvoiceLineApi/getpage`,
    viewData_getlinepagetable_url = `${viewData_baseUrl_PU}/PurchaseInvoiceLineApi/getinvoicelinepage`,
    viewData_getPersonInvoiceCheckExist = `${viewData_baseUrl_PU}/PurchaseInvoiceApi/checkexist`,
    activePageIdDisplay = "purchasePersonInvoiceLineDisplayPage",
    arr_headerLinePagetables = [];


function initDisplayPersonInvoiceFrom(headerPagination = 0, id = 0) {

    pagetable_formkeyvalue[0] = id == 0 ? $(`#${activePageIdDisplay} #personInvoiceId`).val() : id;
    ...
    if (arr_pagetables.findIndex(a => a.pagetable_id == "headerDisplay_pagetable") < 0) {
        ...
        arr_pagetables[index1].pagerowscount = 15
    ...
    if (arr_pagetables.findIndex(a => a.pagetable_id == "lineDisplay_pagetable") < 0) {
        pagetable = {
            ...
            getsum_url: `${viewData_baseUrl_PU}/PurchaseInvoiceLineApi/getLineSum`
        };
        arr_pagetables.push(pagetable);
    } else {
        ...
        arr_pagetables[index2].getsum_url = `${viewData_baseUrl_PU}/PurchaseInvoiceLineApi/getLineSum`
    ...
    get_DisplayPageTable("headerDisplay_pagetable", id => {
        ...
        get_DisplayNewPageTable("lineDisplay_pagetable",
            function () {
                modal_show("displayPersonInvoiceLineModel");
            });
    });
}
...
function display_pagination_display(opr) {
    ...
    var elemId = +$("#purchasePersonInvoiceLineDisplayPage #headerDisplay_pagetable #col_1_1").text();
...
async function display_paginationAsync_display(opr, elemId) {
    ...
    initDisplayPersonInvoiceFrom(headerPagination, elemId)
}
...
function headerindexChoose_display(e) {
    ...
    if (e.keyCode === KeyCode.Enter) {
        ...
        checkExist = checkExistPersonInvoiceLineId(+elm.val());
        if (checkExist) {
            initDisplayPersonInvoiceFrom(0, +elm.val());
        }
        else
...
function checkExistPersonInvoiceLineId(id) {

    let outPut = $.ajax({
        url: viewData_getPersonInvoiceCheckExist,
        async: false,
        type: "post",
        dataType: "json",
        contentType: "application/json",
        ...
        error: function (xhr) {
            error_handler(xhr, viewData_getPersonInvoiceCheckExist);
        }
    });
...
function reset_toParentForm() {
    ...
    // viewData_controllername = "PurchaseInvoiceApi";
}
                    ...
                    if (j == 0) {
                        if (conditionResult != "noCondition") {
                            if (pagetable_highlightrowid != 0 && item[columns[j].id] == pagetable_highlightrowid) {
                                str += '<tr' + primaries + ' class="highlight" id="row' + rowno + '" onkeydown="tr_onclickDisplayDisplay(`' + pageId + '`,this,event)" onclick="tr_onclickDisplay(`' + pageId + '`,this,event)" tabindex="-2"' + `
                             style="${eval(`${item[conditionTools[0].fieldName]} ${conditionTools[0].operator} ${conditionTools[0].fieldValue}`) ? conditionAnswer : conditionElseAnswer}"` + '>';
                                ...
                                str += '<tr' + primaries + ' id="row' + rowno + '" onkeydown="tr_onclickDisplayDisplay(`' + pageId + '`,this,event)" onclick="tr_onclickDisplay(`' + pageId + '`,this,event)" tabindex="-1"' + `
                             style="${eval(`${item[conditionTools[0].fieldName]} ${conditionTools[0].operator} ${conditionTools[0].fieldValue}`) ? conditionAnswer : conditionElseAnswer}"` + '>';
                            }
                        }
                        else {
...
function tr_onclickDisplayDisplay(pg_name, elem, ev) {
...
function afterFillPage(pg_name) {
    ...
    //fillPersonInvoicePurchaseDiaplaylineFooter(id);
...
function fillPersonInvoicePurchaseDiaplaylineFooter(rowIdSelect) {

    getPersonInvoicePurchaselineDiaplayFooter(rowIdSelect).then(function (data) {
        ...
        if (data.length > 0) {
            ...
            $("#fillPersonInvoicePurchaselineFooterTbodyfieldset").removeClass("displaynone");

            strTbodyTh = `
              <div class="detail-headerLine row col-md-12">
                <table style="margin-top:10px;width:100%" id="fillPersonInvoicePurchaselineFooterTbody">
                    <thead >
                        <tr>
            ...
            $(strTbodyTh).appendTo("#header-lines-footer");
            $(strTbodyTh).appendTo("#fillPersonInvoicePurchaselineFooterTbody");
        }
        else
            $("#fillPersonInvoicePurchaselineFooterTbodyfieldset").addClass("displaynone");

    });
}

async function getPersonInvoicePurchaselineDiaplayFooter(id) {
