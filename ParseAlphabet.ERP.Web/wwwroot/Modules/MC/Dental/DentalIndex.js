var viewData_form_title = "لیست دندانداکتری",
    viewData_add_form_title = " نظام دندانداکتری",
    viewData_display_form_title = "نمایش دندانداکتری",
    viewData_edit_form_title = "دندانداکتری - ویرایش",

    viewData_controllername = "DentalApi",
    ...
    viewData_display_page_url = "/MC/Dental/display",
    viewData_add_page_url_Dental_Get = "/MC/Dental/getform",
    viewData_get_DentalType_url = `${viewData_baseUrl_MC}/${viewData_controllername}/gettype`,

    viewData_print_model = { url: "", item: "@Id", value: 0, sqlDbType: 8, size: 0 },
    viewData_print_tableName = "";


function initDentalIndexForm() {
    ...
    pagetable_formkeyvalue = ["myDental", 0];
    get_NewPageTableV1();
...
$("#userType").on("change", function () {
    ...
    if ($(this).prop("checked"))
        ...
        pagetable_formkeyvalue = ["allDental", 0];
...
function DentalGetForm() {
    ...
    if (typeof viewData_add_form_title == "string")
        navigation_item_click(`${viewData_add_page_url_Dental_Get}/null`, viewData_add_form_title);
    else
        navigation_item_click(`${viewData_add_page_url_Dental_Get}/null`, "");
}

function run_button_printDental(dentalId, rowNo, btn) {

    var check = controller_check_authorize(viewData_controllername, "PRN");
    if (!check)
        return;

    viewData_print_model.value = dentalId;
...
function run_button_displayDental(dentalId, rowNo, btn) {

    var check = controller_check_authorize(viewData_controllername, "VIW");
    if (!check)
        return;

    var href = `${viewData_display_page_url}/${dentalId}`;
...
function run_button_editDental(dentalId, rowNo, btn) {
    ...
    if (!check)
        ...
        var href = `${viewData_add_page_url_Dental_Get}/${dentalId}`;
